...
The file is downloaded the first time you run `gitpin` on a given URL. After
that, it checks to see the age of the local file and re-downloads if it is to
old. The default refresh interval is 12 hours, but is configurable with a
parameter.
...
of the file is constructed deterministically based on the URL (specifically, the
base name is the `digest()` of the URL).
...
the desired versioning propertiests. Thus, `gitpins` was born.
